...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("valentineTitle").textContent =
    `${config.valentineName}, ${config.valentineTitle}`;
...
function handleButtonHover(button) {
  ...
  if (buttonText.includes("no")) {
    const customTooltip = button.parentElement.querySelector(".custom-tooltip");
    if (customTooltip) {
      const tooltipMessageIndex = config.no.clickCounter || 0;

      const tooltipMessages = config.no.warningMessages;

      customTooltip.textContent =
        tooltipMessages[tooltipMessageIndex] || "Please don't say no! 😢";
      customTooltip.classList.add("show");
    }
  }
}

function handleButtonLeave(button) {
  const customTooltip = button.parentElement.querySelector(".custom-tooltip");
  if (customTooltip) {
...
function moveButton(button) {
  // Check if button is inside a tooltip wrapper
  const wrapper = button.parentElement.classList.contains("tooltip-wrapper")
    ? button.parentElement
    : button;

  const x = Math.random() * (window.innerWidth - wrapper.offsetWidth);
  const y = Math.random() * (window.innerHeight - wrapper.offsetHeight);
  ...
  if (buttonText.includes("yes")) {
    showNextQuestion(2);
  } else if (buttonText.includes("no")) {
    // Increment noClickCounter
    config.no.clickCounter++;

    // If max clicks reached, show a message
    if (config.no.clickCounter >= config.no.maxClicks) {
      config.no.clickCounter = 0; // Reset counter after showing message
    }
  }
}
